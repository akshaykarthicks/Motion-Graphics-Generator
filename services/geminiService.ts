...
const createPrompt = (settings: FormState): string => {
  ...
  const editInstruction = text.trim()
    ? `While animating, creatively apply the following edits to the image: "${text}". The animation should ideally start with the original image and transition to the edited version.`
    : 'Animate the original image without any edits.';

  return `
    Generate a professional, high-quality motion graphics animation based on the provided source image.

    **Core Creative Brief:**
    - **Base Animation Style:** Apply a "${style}" animation to the image.
    - **Image Edits:** ${editInstruction}
    - **Pacing:** The animation's pacing should be "${pacing}".
    - **Color Palette:** The overall color scheme should be inspired by "${palette}".
    ...
    **Execution Notes:**
    - Ensure all transitions are smooth and professional.
    - The final output must be high-resolution and polished.
